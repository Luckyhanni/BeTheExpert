...
export function BundesligaChampionsLevelOneScreen() {
  ...
  const question = bundesligaChampionsLevelOneQuestions[questionIndex];
  const hasAnswered = selectedOptionId !== null;
  const answeredCorrectly = hasAnswered && isCorrectAnswer(question, selectedOptionId);
  const progress = `${Math.round(((questionIndex + 1) / bundesligaChampionsLevelOneQuestions.length) * 100)}%` as const;
  ...
  function continueQuiz() {
    if (questionIndex === bundesligaChampionsLevelOneQuestions.length - 1) {
      setIsComplete(true);
      return;
    }
  ...
  if (isComplete) {
    const percent = Math.round((correctAnswers / bundesligaChampionsLevelOneQuestions.length) * 100);

    return (
      <AppScreen>
        <View style={styles.resultHeader}>
          <Text style={styles.eyebrow}>LEVEL ABGESCHLOSSEN</Text>
          <Text style={styles.resultTitle}>{correctAnswers} / 30 richtig</Text>
          <Text style={styles.resultPercent}>{percent} %</Text>
        </View>
        <Card style={styles.resultCard}>
          <Text style={styles.resultMessage}>
            {percent >= 80
              ? 'Starke Leistung – du kennst die deutschen Meister.'
              : 'Gute Grundlage. Wiederhole das Level und verbessere deinen Rekord.'}
          </Text>
  ...
  return (
    <AppScreen>
      <View style={styles.header}>
        <Text style={styles.eyebrow}>BUNDESLIGA · MEISTER · LEVEL 1</Text>
        <View style={styles.headerRow}>
          <Text style={styles.title}>Meister erkennen</Text>
          <Text style={styles.counter}>{questionIndex + 1} / 30</Text>
        </View>
      ...
      {hasAnswered ? (
        <Card style={answeredCorrectly ? styles.feedbackCorrect : styles.feedbackWrong}>
          ...
          <PrimaryButton
            label={questionIndex === 29 ? 'Ergebnis anzeigen' : 'Nächste Frage  →'}
      ...
      <Text style={styles.sourceNote}>
        Stand: {bundesligaChampionsLevelOneMeta.verifiedAsOf} · Historische deutsche Meister seit 1903
      </Text>
    </AppScreen>
  );
}
